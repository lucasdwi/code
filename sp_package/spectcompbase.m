...
[LFPTs,chk_nan,zeroedChannel,nanInd,clnTrls,trls,adfreq] = preProcess(LFPTs,...
...
if strcmpi(cohMethod,'mat')
    ...
    [coh,cohPlots] = cohComp(trls,adfreq,eoi,bands,zeroedChannel,foi,...
        nFilt,vis,cohMethod,discrete,nanInd,'overlap',overlap);
    toc
elseif strcmpi(cohMethod,'mtm')
    tic
    disp('Using cmtm.m to calculate coherence...')
    [coh,cohPlots] = cohComp(trls,adfreq,eoi,bands,zeroedChannel,foi,...
        nFilt,vis,cohMethod,discrete,nanInd,'NW',8);
    toc
end
...
hist.foi = foi; hist.fixed = fixed; hist.discrete = discrete;
hist.bands = bands; hist.overlap = 1-overlap; hist.cohMethod = cohMethod; 
hist.eoi = eoi; hist.saveParent = saveParent; hist.adfreq = adfreq; 
hist.chk_nan = chk_nan; hist.eventTs = eventTs; hist.nanInd = nanInd;
