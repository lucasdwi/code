function [data,rawPowArr,meanPowArr,relPowArr,rawCohArr,meanCohArr,relCohArr,nTrls,relPowAvg,relCohAvg,powCorrArr] = collateData(sdir,searchStr)
...
powCorrArr = cell(numel(fileStruct),nEvents);
...
for fsi = 1:numel(fileStruct)
    ...
    for fi = 1:size(fileStruct{fsi},1)
        ...
        if isfield(psdTrls,'event2')
            pow{1,2} = struct2cell(psdTrls.event2);
        end
        ...
        for ei = 1:size(pow,2)
            ...
            % Grab powerCorr
            powCorrArr{fsi,ei} = [powCorrArr{fsi,ei};powerCorrSort{1,ei}.masterCorrMeanPlot(~isnan(powerCorrSort{1,ei}.masterCorrMeanPlot))'];
...
for fsi = 1:numel(fileStruct)
    for ei = 1:nEvents
        ...
        data{fsi,ei} = [thisPow,thisCoh,powCorrArr{fsi,ei}];
    end
end
