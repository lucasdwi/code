...
% E.G. spectcomp('conditionxanimal1','conditionyanimal1',6,6,[-8 2],[-8 2],[1 2 150],0.5,
...
% be constructed with 1 to 150 Hz in steps of 2 Hz using 0.5 second
% windows, between 8 seconds before the start of the both binges and 2 seconds
% after at 0.05 second intervals, 
%% First run Matt and Wilder's code, PL2tomvdm.m, to convert pl2 data into
% usable data structures
[fname,pl2,n,freqs,j,lfpchan,ad,adfreq,fn,i,LFPTs,temp,TimeSampEr,ts,WBchan,eventTs]= Pl2tomvdm(file1);
...
if tf == 0
    [fname,pl2,n,freqs,j,lfpchan,ad,adfreq,fn,i,LFPTs,temp,TimeSampEr,ts,WBchan,eventTs]= Pl2tomvdm(file2);
...
% Create Trials
cfg = [];
cfg.t = eventTs.t{1,ind(event2)}; % Timestamps corresponding to event 1
cfg.twin = twin1; % time window around event 1
