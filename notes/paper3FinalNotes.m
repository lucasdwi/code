...
for ii = 1:nFiles
    ...
    day{ii,1} = repmat({parts{4}},n(ii,1),1); %#ok
end
...
for ii = 1:nFiles
    ...
    for jj = 1:2
        ...
        day{ii,jj} = repmat({parts{4}},n(ii,jj),1); %#ok
    end
end
% Concatenate
...
% Combine
postData = [thisAlc,thisWat,thisNeither]; %#ok
% save('D:\paper3\analyzed\final\waterAlcoholNotData.mat','alc','allData',...
%     'notD','watAlc','water','notWater','postData')
%% Feeding data
...
% steps = [1,1.5,2,4,6,8,16,32,45,64];
steps = 1;
nSteps = numel(steps);
nGen = 232;
nF = 150;
...
c = 1;
for jj = steps
    cd(['F:\paper3\analyzed\final\genModel\',num2str(jj),'feedWeightLOO'])
    for ii = 1:81
...
% Plot ROCs; old code, needs to be adjusted for added 3rd dimension
%     figure
%     hold on
%     plot(mean(genX,1),mean(genY,1),'-k')
%     plot(mean(genXR,1),mean(genYR,1),':k')
%     plot(mean(dX,1),mean(dY,1),'--k')
%     plot(mean(fX,1),mean(fY,1),'-.k')
%     legend({['Gen: ',num2str(round(mean(genAcc(:,c)),2)),'\pm',...
%         num2str(round(conf(genAcc(:,c)',0.95),2))],...
%         ['GenP: ',num2str(round(mean(genAccR(:,c)),2)),'\pm',...
%         num2str(round(conf(genAccR(:,c)',0.95),3))],...
%         ['Drink: ',num2str(round(mean(dAcc(:,c)),2)),'\pm',...
%         num2str(round(conf(dAcc(:,c)',0.95),2))],...
%         ['Feed: ',num2str(round(mean(fAcc(:,c)),2)),'\pm',...
%         num2str(round(conf(fAcc(:,c)',0.95),2))]},'location','se')
%     set(gca,'xtick',0:0.5:1,'ytick',0:0.5:1)
%     xlabel('FPR')
%     ylabel('TPR')
%     title(['Generalized model: shell features only (',...
%         num2str(jj),'feed weight)'])
%% Plot results of gen model ()
this = 1;
% figure
% hold on
% h(1) = histogram(genAcc(:,5),'normalization','probability');
% h(2) = histogram(genAccR(:,5),'normalization','probability');
% h(3) = histogram(fAcc(:,5),'normalization','probability');
% h(4) = histogram(dAcc(:,5),'normalization','probability');
% xax = get(gca,'xlim');
% len = xax(2)-xax(1);
% % Split into 40 bins
% bin = len/40;
...
cd F:\paper3\analyzed\final\feedOtherAllLOO\
for ii = 1:100
...
cd F:\paper3\analyzed\final\drinkOtherAllLOO\
[x,y,xR,yR] = deal(cell(100,9));
for ii = 1:100
   load(['drinkOtherAllLOO',num2str(ii),'.mat'],'acc','accR','hist',...
       'histR')
   for jj = 1:9
...
% Plot auc histograms
% doubleHist(fA,dA)
% legend({['Feed: \mu = ',num2str(round(mean(fA),2)),'\pm',...
%     num2str(round(conf(fA,0.95),2))],...
%     ['Drink: \mu = ',num2str(round(mean(dA),2)),'\pm',...
%     num2str(round(conf(dA,0.95),2))]},...
%     'Location','nw');
doubleHist(fA,fAR)
...
% Feeding
cd F:\paper3\analyzed\final\feedOtherLOO\
for ii = 1:100
   ...
   % Apply to drink data
   load(['F:\paper3\analyzed\final\drinkOtherLOO\drinkOtherLOO',num2str(ii),...
       '.mat'],'hist')
   for jj = 1:9
...
% Drinking
cd F:\paper3\analyzed\final\drinkOtherLOO\
[x,y,xR,yR] = deal(cell(100,9));
for ii = 1:100
   ...
   % Apply to feed data
    load(['F:\paper3\analyzed\final\feedOtherLOO\feedOtherLOO',num2str(ii),...
       '.mat'],'hist')
   for jj = 1:9
       pred = cvglmnetPredict(acc{jj}{1}.mdl{1},...
           hist{jj}.cfg.naive.testX,'lambda_1se','response');
...
%      'mdfY','dfA','fdA')
%% Plot ROCs
load('F:\paper3\analyzed\final\feedAlcoholOtherSubsetModelsLOO.mat')
% Within models
% ROCs
figure
hold on
...
% % Feed
% for ii = 1:100
%     load(['F:\paper3\analyzed\final\feedOther\feedOther',num2str(ii),...
%         '.mat'])
%     [fX(ii,:),fY(ii,:),~,fA(ii)] = perfcurve(hist.cfg.naive.testY,...
%         acc{1}.pred,1);
%     [thisX,thisY,~,fAR(ii)] = perfcurve(histR.cfg.naive.testY,...
...
%         linspace(0,1,142));
%     % Apply to drink data
%     load(['F:\paper3\analyzed\final\drinkOther\drinkOther',num2str(ii),...
%         '.mat'],'hist')
...
% xlabel('AUC'); ylabel('probability')1
% title('behavior vs. not+other; subset features')
...
[s,a] = deal(zeros(81,18));
for ii = 1:81
    load(['F:/paper3/analyzed/final/genModel/1feedWeight/'...
...
cd F:/paper3/analyzed/final/maleFemaleDrink/
...
[maleX,maleY,femaleX,femaleY] = deal(zeros(100,5,21));
for ii = 1:100
...
legend({['male: ',num2str(round(mean(mean(maleAUC)),2)),'\pm',...
    num2str(round(conf(reshape(maleAUC,1,500),0.95),2))],...
    ['female: ',num2str(round(mean(mean(femaleAUC)),2)),'\pm',...
    num2str(round(conf(reshape(femaleAUC,1,500),0.95),2))]},...
    'location','se')
title('Intra-sex models')
xlabel('FPR'); ylabel('TPR')
set(gca,'xtick',0:0.5:1,'ytick',0:0.5:1)

load('F:\paper3\analyzed\final\feedAlcoholOtherFullModelsLOO.mat',...
    'mDrinkX','mDrinkY','dA')
figure
hold on
...
legend({['m->f: ',num2str(round(mean(mean(mean(mfAUC))),2)),'\pm',...
    num2str(round(conf(reshape(mfAUC,1,2500),0.95),3))],...
    ['f->m: ',num2str(round(mean(mean(mean(fmAUC))),2)),'\pm',...
    num2str(round(conf(reshape(fmAUC,1,2500),0.95),3))],...
    ['all: ',num2str(round(mean(dA),2)),'\pm',...
    num2str(round(conf(dA,0.95),2))]},'location','se')
title('Inter-sex models')
xlabel('FPR'); ylabel('TPR')
set(gca,'xtick',0:0.5:1,'ytick',0:0.5:1)
%%
figure
