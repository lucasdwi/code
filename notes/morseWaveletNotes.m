...
for ii = 1:nChan
    ...
    % Reset color limits so COI does not change underneath colors
    set(gca,'Clim',c)
...
%% Calculate Coherence
cmb = nchoosek(1:nChan,2);
wCoh = zeros(lData,size(f,1),size(cmb,1));
figure
for ii = 1:size(cmb,1)
    wCoh(:,:,ii) = abs(sCross{cmb(ii,1)}(:,:,cmb(ii,2))).^2./(sAuto(:,:,cmb(ii,1)).*sAuto(:,:,cmb(ii,2)));
    subplot(3,2,ii)
    imagesc(t,f,wCoh(:,:,ii)')
    colormap('viridis')
    set(gca,'YDir','normal')
    ...
    % Reset color limits so COI does not change underneath colors
    set(gca,'Clim',c)
    title([num2str(cmb(ii,1)),'-',num2str(cmb(ii,2))])
    xlabel('Time (sec)')
    ylabel('Frequency (Hz)')
end
