function [s, c, ph, ci, phi] = cmtm(x,y,dt,NW,qbias,confn,qplot)
%
% Multi-taper method coherence using adaptive weighting and correcting
% for the bias inherent to coherence estimates.  The 95% coherence
% confidence level is computed by cohconf.m.  In addition, a built-in
% Monte Carlo estimation procedure is available to estimate phase 95%
% confidence limits.
%
% Inputs:
%         x     - Input data vector 1.
%         y     - Input data vector 2.
%         dt    - Sampling interval (default 1)
%         NW    - Number of windows to use (default 8)
%         qbias - Correct coherence estimate for bias (yes, 1)  (no, 0, 
%                 default).
%         confn - Number of iterations to use in estimating phase 
%                 uncertainty using a Monte Carlo method. (default 0)
%         qplot - Plot the results, (yes, 1), (No, 0, default).  The upper
%                 tickmarks indicate the bandwidth of the coherence and 
%                 phase estimates.
%
% Outputs:
%         s       - frequency
%         c       - coherence
%         ph      - phase
%         ci      - 95% coherence confidence level
%         phi     - 95% phase confidence interval, bias corrected
%                   (add and subtract phi from ph).
%
%
% Required files: cohconf.m, cohbias.m, cohbias.mat, Matlab signal
% processing toolbox.
%__________________________________________________________________________
% Peter Huybers
% MIT, 2003
% phuyber@mit.edu
%
% Modified by LLD 2018
%__________________________________________________________________________
%%
% Check input
if nargin<2
    help cmtm
    return
end
if nargin<7
    qplot=0;
end
if nargin<6
    confn=0;
end
if nargin<5
    qbias=0;
end
if nargin<4
    NW=8;
end
if isempty(NW)
    NW=8;
end
if nargin<3
    dt=1;
end
if isempty(dt)
    dt=1;
end
if NW<1.5
    disp('Warning: NW must be greater or equal to 1.5');
    return
end
% Normalize x and y
x=x(:)-mean(x);
y=y(:)-mean(y);
% Make sure x and y are the same size
if length(x)~=length(y)
    disp('Warning: the lengths of x and y must be equal.');
    return
end
% Define some parameters
N = length(x);
k = min(round(2*NW),N);
k = max(k-1,1);
s = (0:1/(N*dt):1/dt-1/(N*dt))';
pls = 2:(N+1)/2+1;
% Approximate degrees of freedom
v = (2*NW-1);
if rem(length(y),2)==1
    pls = pls(1:end-1);
end
% Compute the discrete prolate spheroidal sequences, requires the spectral
% analysis toolbox.
[E,lambda]=dpss(N,NW,k);
% Compute the windowed DFTs.
fkx=fft(E(:,1:k).*x(:,ones(1,k)),N);
fky=fft(E(:,1:k).*y(:,ones(1,k)),N);
% Calculate power spectra
Pkx=abs(fkx).^2;
Pky=abs(fky).^2;
% Itteration to determine adaptive weights:
for ii = 1:2
    if ii == 1
        vari = x'*x/N;
        Pk = Pkx;
    end
    if ii == 2
        vari = y'*y/N;
        Pk = Pky;
    end
    % Initial spectral estimate
    P = (Pk(:,1)+Pk(:,2))/2;
    % Set up Ptemp and P1
%     Ptemp = zeros(N,1);
    P1 = zeros(N,1);
    % Set up tolerance; should reach within 3 iterations
    tol = 0.0005*vari/N;
    % Set up variance measure
    a = vari*(1-lambda);
    while sum(abs(P-P1)/N)>=tol
        % Weights
        b = (P*ones(1,k))./(P*lambda'+ones(N,1)*a');
        % New spectral estimate
        wk = (b.^2).*(ones(N,1)*lambda');
        P1 = (sum(wk'.*Pk')./ sum(wk'))';
        % Swap P and P1
        Ptemp = P1;
        P1 = P;
        P = Ptemp;
    end
    if ii == 1
        fkx = sqrt(k)*sqrt(wk).*fkx./repmat(sum(sqrt(wk'))',1,k);
        % PSD of x
        Fx = P;
    end
    if ii == 2
        fky = sqrt(k)*sqrt(wk).*fky./repmat(sum(sqrt(wk'))',1,k);
        % PSD of y
        Fy = P;
    end
end
% As a check, the quantity sum(abs(fkx(pls,:))'.^2) is the same as Fx and
% the spectral estimate from pmtmPH.

% Compute coherence
Cxy = sum([fkx.*conj(fky)]');
ph = angle(Cxy)*180/pi;
c = abs(Cxy)./sqrt(sum(abs(fkx').^2).*sum(abs(fky').^2));
% Imaginary coherence
% % c = imag(Cxy./sqrt(sum(abs(fkx').^2).*sum(abs(fky').^2)));

% Correct for the bias of the estimate
if qbias == 1
    c = cohbias(v,c)';
end

% Phase uncertainty estimates via Monte Carlo analysis.
if confn>1
    cb = cohbias(v,c)';
    for iter = 1:confn
        if rem(iter,10) == 0
            disp(['phase confidence iteration: ',num2str(iter)]);
        end
        fx = fft(randn(size(x))+1);
        fx = fx/sum(abs(fx));
        fy = fft(randn(size(y))+1);
        fy = fy/sum(abs(fy));
        ys = real(ifft(fy.*sqrt(1-cb'.^2)));
        ys = ys+real(ifft(fx.*cb'));
        xs = real(ifft(fx));
        [si, ciph(iter,:), phi(iter,:)] = cmtm(xs,ys,dt,NW);
    end
    % Sorting and averaging to determine confidence levels.
    pl = round(.975*iter);
    phi = sort(phi);
    phi = [phi(pl,:); -phi(iter-pl+1,:)];
    phi = mean(phi);
    phi = conv(phi(1:end),[1 1 1]/3);
    phi = phi(2:end-1);
else
    phi = zeros(size(pls));
end

%Cut to one-sided funtions
c = c(pls);
s = s(pls)';
ph = ph(pls);
phl = ph-phi;
phu = ph+phi;

%Coherence confidence level
ci = cohconf(v,.95);  %not corrected for bias, this is conservative.
ci = ci*ones(size(c));

% Plotting
if qplot == 1
    % Coherence
    figure(gcf); 
    clf;
    subplot(211); 
    hold on;
    plot(s,c);
    h = ylabel('coherence');
    h = xlabel('frequency');
    plot(s,ci,'k--');
    pl = find(c>ci(1));
    title(['mean is ',num2str(mean(c),2),'   ',...
        num2str(100*length(pl)/length(c),2),...
        '% of estimates above 95% confidence level']);
    axis tight; 
    h=axis; 
    axis([h(1:2) 0 1.025]);
    % Half-bandwidth of the dpss
    w = NW/(dt*N);
    plot([s(1) h(2)],[1.02 1.02],'k');
    for ds = min(s):2*w:max(s)
        plot([ds ds],[.98 1.02],'k');
    end
    % Phase
    subplot(212); 
    hold on;
    plot(s,ph);
    if confn>0
        col = [.9 .9 .9];
        h = fill([s(1) s(1:end) fliplr([s(1:end) s(end)])],...
            [phu(1) phl fliplr([phu phl(end)])],col);
        set(h,'edgecolor',col);
        
        pl = find(phu<=180); 
        phu(pl) = -180;
        pl = find(phu> 180); 
        phu(pl) = phu(pl)-360;
        phlt = -180*ones(size(phl));
        h = fill([s(1) s(1:end) fliplr([s(1:end) s(end)])],...
            [phu(1) phlt fliplr([phu phlt(end)])],col);
        set(h,'edgecolor',col);
        
        pl = find(phl>=-180); 
        phl(pl) = 180;
        pl = find(phl< -180); 
        phl(pl) = phl(pl)+360;
        phut = 180*ones(size(phl));
        h = fill([s(1) s(1:end) fliplr([s(1:end) s(end)])],...
            [phut(1) phl fliplr([phut phl(end)])],col);
        set(h,'edgecolor',col);
    end
    h = plot(s,ph);
    plot(s,zeros(size(s)),'k--');
    axis tight; 
    h = axis; 
    axis([h(1:2) -180 180]);
    h = xlabel('frequency');
    h = ylabel('phase');
end



